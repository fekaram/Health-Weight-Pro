...
`)}}function Be(e){let t=String(e??``).trim().toLowerCase();return Re.find(e=>e.toLowerCase()===t)??`meal`}function j(e){if(e==null||e===``)return null;let t=Number(e);return Number.isFinite(t)&&t>=0?t:null}function Ve({root:e,viewModel:t}){let n=t.getState();e.innerHTML=`
  ...
  `}function rt({activeDialog:e,today:t,settings:n,routes:r,lastImportedMeal:i,i18n:a}){return e===`water`?dt(t,n,a):e===`sleep`?pt(t,n,a):e===`steps`?mt(t,n,a):e===`meal-register`?at(r,a):e===`nutri-ia`?ot(a):e===`nutri-ia-help`?st(a):e===`nutri-ia-success`?ct(i,a):it(r,a)}function it(e,t){let n=e.find(e=>e.id===`medication`);return`
    <ul class="bottom-sheet__options">
      ...
      ${n?`<li><a class="bottom-sheet__option" href="${n.path}" data-route="${n.path}"><span aria-hidden="true">💉</span><span>${t.t(`habit.medication`)}</span></a></li>`:``}
    </ul>
  ...
  `}function at(e,t){let n=e.find(e=>e.id===`favorites`),r=e.find(e=>e.id===`meal-plans`),i=e.find(e=>e.id===`food-library`);return`
    ...
    <ul class="bottom-sheet__options">
      ...
      ${n?`<li><a class="bottom-sheet__option" href="${n.path}" data-route="${n.path}"><span aria-hidden="true">⭐</span><span>${t.t(`fab.favorites`)}</span></a></li>`:``}
      ${r?`<li><a class="bottom-sheet__option" href="${r.path}" data-route="${r.path}"><span aria-hidden="true">🍽️</span><span>${t.t(`fab.mealPlans`)}</span></a></li>`:``}
      ${i?`<li><a class="bottom-sheet__option" href="${i.path}" data-route="${i.path}"><span aria-hidden="true">📚</span><span>${t.t(`fab.foodLibrary`)}</span></a></li>`:``}
    </ul>
  ...
  `}function ar(e){let t=e.editingRecord??{},n=e.app.i18n,r=e.app.units.measurement;return`
  ...
  `}function or(e,t){return e.length===0?K(t.t(`body.empty`)):`
  ...
  `}function sr(e,t){let n={chest:t.t(`body.chest`),waist:t.t(`body.waist`),hips:t.t(`body.hips`),arm:t.t(`body.arm`),thigh:t.t(`body.thigh`)};return[`chest`,`waist`,`hips`,`arm`,`thigh`].filter(t=>e[t]!==null&&e[t]!==void 0).map(r=>`${y(n[r])}: ${y(t.formatNumber(e[r]))}`).join(` &middot; `)}var cr=[{key:`calories`,labelKey:`dashboard.calories`,unit:`kcal`,icon:`flame`},{key:`protein`,labelKey:`dashboard.protein`,unit:`g`,icon:`protein`},{key:`water`,labelKey:`dashboard.water`,unit:`mL`,icon:`water`},{key:`sleep`,labelKey:`dashboard.sleep`,unit:`h`,icon:`moon`},{key:`steps`,labelKey:`dashboard.steps`,icon:`steps`},{key:`weight`,labelKey:`dashboard.weight`,icon:`scale`},{key:`medication`,labelKey:`dashboard.medication`,icon:`syringe`},{key:`recentMeals`,labelKey:`dashboard.recentMealsCard`,icon:`utensils`},{key:`recentActivity`,labelKey:`dashboard.recentActivityCard`,icon:`activity`}];function lr(e){let t=e.summary??{},n=e.app.settings??{},r=e.app.i18n,i=e.justAchievedKeys??[],a=e.allGoalsJustAchieved??!1,o=dr({summary:t,settings:n,i18n:r,units:e.app.units,justAchievedKeys:i}),s=o.filter(e=>e.key===`medication`&&n.medication?.enabled===!1?!1:n.dashboardCards?.[e.key]??!0),c=s.filter(e=>e.achievable),l=c.length>0&&c.every(e=>e.achieved);return`
  ...
  `}function mr(e,t){return`
    <article class="metric-card ${[`metric-card--${e.status}`,e.justAchieved?`metric-card--celebrate`:``].filter(Boolean).join(` `)}">
      <div class="metric-card__header">
        <span class="metric-card__icon" aria-hidden="true">${vr(e.icon)}</span>
  ...
  `}var yr=[`details`,`items`,`preview`],br=[{value:`mostUsed`,icon:`⭐`,labelKey:`favorites.sort.mostUsed`},{value:`recentlyUsed`,icon:`🕒`,labelKey:`favorites.sort.recentlyUsed`},{value:`alphabetical`,icon:`🔤`,labelKey:`favorites.sort.alphabetical`}];function X(e,t){return e===`custom`?t.t(`favorites.category.custom`):t.t(`mealSlot.${e}`)}function xr(e){return[...Fe(e),`custom`]}function Sr(e){let t=e.app.i18n,n=e.app.settings??{},r=e.favoriteMeals??[],i=e.foods??[],a=r.length===0;return`
    <section class="module-screen" aria-labelledby="favorites-title">
      ...
      ${a?wr(t):Tr(r,i,e.sortOption,n,e.message,t)}

      ${e.activeDialog===`wizard`&&e.draft?Or(e,n,t):``}
      ${e.activeDialog===`delete-confirm`?Fr(r.find(t=>t.id===e.selectedFavoriteId),t):``}
    </section>
  `}function Cr(e,t){let n=t.app.i18n,r=t.foods??[];if(zr(e,t.favoriteMeals??[],r,n),t.activeDialog===`wizard`&&t.wizardStep===`items`)if(t.pickingFood){let r=t.editingItemIndex===null?null:t.draft.items[t.editingItemIndex],i=r?r.quantity:t.pickingFood.servingSize;Vr(e,t.pickingFood,i,n)}else Br(e,r,n)}function wr(e){return`
  ...
  `}function Tr(e,t,n,r,i,a){return`
    <section class="content-panel">
      ${W(i,`success`,a)}
      ...
      <div class="favorites__toolbar">
        ...
        <select class="favorites__category-filter" data-favorite-category-filter aria-label="${a.t(`favorites.category`)}">
          ...
          ${xr(r).map(e=>`<option value="${e}">${X(e,a)}</option>`).join(``)}
        </select>
  ...
  `}function Er(e,t,n){return e.length===0?K(n.t(`favorites.noResults`)):e.map(e=>Dr(e,t,n)).join(``)}function Dr(e,t,n){let r=z(e,t);return`
  ...
  `}function Or(e,t,n){let{draft:r,wizardMode:i,wizardStep:a}=e,o=n.t(i===`edit`?`favorites.editFavorite`:`favorites.createFavorite`),s=yr.indexOf(a);return`
    ...
    <div class="bottom-sheet favorites-wizard" role="dialog" aria-modal="true" aria-label="${o}">
      ...
      <div class="favorites-wizard__steps" aria-hidden="true">
        ${yr.map((e,t)=>`<span class="favorites-wizard__step${t<=s?` is-active`:``}"></span>`).join(``)}
      </div>
      ${a===`details`?kr(r,e.errors,t,n):a===`items`?Ar(e,n):jr(e,n)}
    </div>
  `}function kr(e,t,n,r){return`
    <form class="module-form" data-module="favorites" data-action="saveDetails">
      ...
      <label>
        ...
        <select name="category">${Ir(e.category,n,r)}</select>
      </label>
      <div class="form-actions">
  ...
  `}function Ar(e,t){let{draft:n,foods:r,pickingFood:i,editingItemIndex:a}=e;if(i){let e=a===null?null:n.items[a],r=e?e.quantity:i.servingSize;return`
      <div class="favorites-wizard__section">
        ...
        <form class="module-form module-form--single" data-module="favorites" data-action="addItem">
          ...
          <label>
            <span>${t.t(`foodLibrary.quantity`)} (${y(Lr(i.servingUnit,t))})</span>
  ...
  `}function jr(e,t){let{draft:n,foods:r,totals:i}=e;return`
    <div class="favorites-wizard__section">
  ...
  `}function Mr(e,t){return e.length===0?K(t.t(`foodLibrary.noResults`)):e.map(e=>`
        <article class="food-card food-card--compact" data-module="favorites" data-action="openFoodPicker" data-id="${e.id}" role="button" tabindex="0">
          ...
          <p class="food-card__serving">${t.formatNumber(e.servingSize)} ${y(Lr(e.servingUnit,t))}</p>
        </article>
      `).join(``)}function Nr(e,t,n){return!e||e.length===0?K(n.t(`favorites.noFoodsAddedYet`)):`
    <div class="favorites-item-list">
      ${e.map((e,r)=>{let i=ln(t,e);return`
            <div class="favorites-item-row">
              <div>
                <strong>${y(i?Rr(i,n):n.t(`common.noRecord`))}</strong>
                <span>${n.formatNumber(e.quantity)} ${y(Lr(e.servingUnit,n))}</span>
              </div>
  ...
  `:``}function Ir(e,t,n){let r=xr(t);return(r.includes(e)?r:[...r,e]).map(t=>`<option value="${t}" ${b(e,t)}>${X(t,n)}</option>`).join(``)}function Lr(e,t){return e===`g`||e===`mL`?e:t.t(`foodLibrary.unit.${e}`)}function Rr(e,t){return e.isBuiltIn&&e.foodCode&&(e.name===I(e.foodCode,`en`)||e.name===I(e.foodCode,`pt-BR`))?I(e.foodCode,t.language)??e.name:e.name}function zr(e,t,n,r){let i=e?.querySelector(`[data-favorite-search]`),a=e?.querySelector(`[data-favorite-category-filter]`),o=e?.querySelector(`[data-favorite-list]`);if(!i||!o)return;let s=()=>{let e=i.value.trim().toLowerCase(),s=a?.value??`all`,c=t.filter(t=>{let n=s===`all`||t.category===s,i=X(t.category,r).toLowerCase(),a=t.name.toLowerCase(),o=!e||a.includes(e)||i.includes(e);return n&&o});o.innerHTML=Er(c,n,r)};i.addEventListener(`input`,s),a?.addEventListener(`change`,s)}function Br(e,t,n){let r=e?.querySelector(`[data-favorite-food-search]`),i=e?.querySelector(`[data-favorite-food-results]`);!r||!i||r.addEventListener(`input`,()=>{let e=r.value.trim().toLowerCase(),a=t.filter(t=>{let r=n.t(`foodLibrary.category.${t.category}`).toLowerCase(),i=Rr(t,n).toLowerCase();return!e||i.includes(e)||r.includes(e)});i.innerHTML=Mr(a,n)})}function Vr(e,t,n,r){let i=e?.querySelector(`[data-favorite-quantity-input]`),a=e?.querySelector(`[data-favorite-quantity-preview]`);if(!i||!a)return;let o=()=>{let e=Number(i.value)||0,n=t.servingSize>0?e/t.servingSize:0;a.innerHTML=Pr({calories:t.calories*n,protein:t.protein*n,carbs:t.carbs*n,fat:t.fat*n,fiber:t.fiber*n},r)};i.addEventListener(`input`,o),o()}function Hr(e){let t=e.app.i18n,n=e.foods??[],r=n.length===0;return`
    <section class="module-screen" aria-labelledby="food-library-title">
      ...
      ${r?Wr(t):Gr(n,e.message,t)}

      ${e.activeDialog===`form`?Jr(e,t):``}
      ${e.activeDialog===`quantity`&&e.selectedFood?Yr(e.selectedFood,t):``}
      ${e.activeDialog===`delete-confirm`&&e.selectedFood?Xr(e.selectedFood,t):``}
    </section>
  `}function Ur(e,t){let n=t.app.i18n;ti(e,t.foods??[],n),t.activeDialog===`quantity`&&t.selectedFood&&ni(e,t.selectedFood,n)}function Wr(e){return`
  ...
  `}function Kr(e,t){return e.length===0?K(t.t(`foodLibrary.noResults`)):e.map(e=>qr(e,t)).join(``)}function qr(e,t){return`
    <article class="food-card" data-module="food-library" data-action="openQuantity" data-id="${e.id}" role="button" tabindex="0">
      <div class="food-card__header">
        <h3>${y(Z(e,t))}</h3>
        <span class="food-card__category">${y(t.t(`foodLibrary.category.${e.category}`))}</span>
      </div>
      <p class="food-card__serving">${t.formatNumber(e.servingSize)} ${y($r(e.servingUnit,t))}</p>
  ...
  `}function Jr(e,t){let n=e.editingFood??{},r=!!e.editingFood;return`
    ...
    <div class="bottom-sheet" role="dialog" aria-modal="true" aria-label="${t.t(r?`foodLibrary.editFood`:`foodLibrary.addFood`)}">
      ...
      <form class="module-form" data-module="food-library" data-action="save">
        ...
        <label>
          ...
          <select name="category">${Zr(n.category??`others`,t)}</select>
        </label>
        <label>
        ...
        <label>
          ...
          <select name="servingUnit">${Qr(n.servingUnit??`g`,t)}</select>
        </label>
        <label>
  ...
  `}function Yr(e,t){let n=$r(e.servingUnit,t);return`
    <button type="button" class="bottom-sheet-overlay" data-module="food-library" data-action="closeDialog" aria-label="${t.t(`common.close`)}"></button>
  ...
  `}function Zr(e,t){return Nt.map(n=>`<option value="${n}" ${b(e,n)}>${t.t(`foodLibrary.category.${n}`)}</option>`).join(``)}function Qr(e,t){return Pt.map(n=>`<option value="${n}" ${b(e,n)}>${$r(n,t)}</option>`).join(``)}function $r(e,t){return e===`g`||e===`mL`?e:t.t(`foodLibrary.unit.${e}`)}function Z(e,t){return e.isBuiltIn&&e.foodCode&&(e.name===I(e.foodCode,`en`)||e.name===I(e.foodCode,`pt-BR`))?I(e.foodCode,t.language)??e.name:e.name}function ei(e,t){return`
    <div class="food-preview-card">
      <dl class="food-preview-grid">
        <div><dt>${t.t(`settings.calories`)}</dt><dd>${t.formatNumber(e.calories)} kcal</dd></div>
        <div><dt>${t.t(`settings.protein`)}</dt><dd>${t.formatNumber(e.protein)} g</dd></div>
  ...
  `}function ti(e,t,n){let r=e?.querySelector(`[data-food-search]`),i=e?.querySelector(`[data-food-category-filter]`),a=e?.querySelector(`[data-food-list]`);if(!r||!a)return;let o=()=>{let e=r.value.trim().toLowerCase(),o=i?.value??`all`,s=t.filter(t=>{let r=o===`all`||t.category===o,i=n.t(`foodLibrary.category.${t.category}`).toLowerCase(),a=Z(t,n).toLowerCase(),s=!e||a.includes(e)||i.includes(e);return r&&s});a.innerHTML=Kr(s,n)};r.addEventListener(`input`,o),i?.addEventListener(`change`,o)}function ni(e,t,n){let r=e?.querySelector(`[data-quantity-input]`),i=e?.querySelector(`[data-quantity-preview]`);if(!r||!i)return;let a=()=>{let e=Number(r.value)||0,a=t.servingSize>0?e/t.servingSize:0;i.innerHTML=ei({calories:t.calories*a,protein:t.protein*a,carbs:t.carbs*a,fat:t.fat*a,fiber:t.fiber*a},n)};r.addEventListener(`input`,a),a()}function ri(e){let t=e.editingRecord??{},n=e.app.i18n,r=Fe(e.app.settings??{}),i=t.mealType&&!r.includes(t.mealType)?[...r,t.mealType]:r;return`
  ...
  `}function ii(e,t){return e.length===0?K(t.t(`meal.empty`)):`
    <div class="record-list">
      ${e.map(e=>`
            <article class="record-card">
              <div>
  ...
  `}var ai=[`details`,`meals`,`preview`];function oi(e){let t=e.app.i18n,n=e.mealPlans??[],r=e.favoriteMeals??[],i=e.foods??[],a=n.length===0;return`
    <section class="module-screen" aria-labelledby="meal-plans-title">
      ...
      ${a?ci(t):li(n,r,i,e.message,t)}

      ${e.activeDialog===`wizard`&&e.draft?fi(e,e.app.settings??{},t):``}
      ${e.activeDialog===`delete-confirm`?yi(n.find(t=>t.id===e.selectedPlanId),t):``}
    </section>
  `}function si(e,t){let n=t.app.i18n,r=t.favoriteMeals??[],i=t.foods??[];bi(e,t.mealPlans??[],r,i,n),t.activeDialog===`wizard`&&t.wizardStep===`meals`&&!t.pickingFavoriteMeal&&xi(e,r,i,n)}function ci(e){return`
  ...
  `}function ui(e,t,n,r){return e.length===0?K(r.t(`mealPlans.noResults`)):e.map(e=>di(e,t,n,r)).join(``)}function di(e,t,n,r){let i=Dn(e,t,n);return`
  ...
  `}function fi(e,t,n){let{draft:r,wizardMode:i,wizardStep:a}=e,o=n.t(i===`edit`?`mealPlans.editPlan`:`mealPlans.createPlan`),s=ai.indexOf(a);return`
    ...
    <div class="bottom-sheet favorites-wizard" role="dialog" aria-modal="true" aria-label="${o}">
      ...
      <div class="favorites-wizard__steps" aria-hidden="true">
        ${ai.map((e,t)=>`<span class="favorites-wizard__step${t<=s?` is-active`:``}"></span>`).join(``)}
      </div>
      ${a===`details`?pi(r,e.errors,n):a===`meals`?mi(e,t,n):hi(e,n)}
    </div>
  ...
  `}function mi(e,t,n){let{draft:r,favoriteMeals:i,foods:a,pickingFavoriteMeal:o,editingMealIndex:s}=e;if(o){let e=s===null?null:r.meals[s],i=Fe(t),c=e?.mealSlot&&!i.includes(e.mealSlot)?[...i,e.mealSlot]:i,l=z(o,a);return`
      <div class="favorites-wizard__section">
        <h3>${y(o.name)}</h3>
  ...
  `}function hi(e,t){let{draft:n,favoriteMeals:r,totals:i}=e;return`
    <div class="favorites-wizard__section">
      <p class="favorites-preview__name">${y(n.name)}</p>
  ...
  `}function gi(e,t,n){return e.length===0?K(n.t(`favorites.noResults`)):e.map(e=>{let r=z(e,t);return`
      ...
      `}).join(``)}function _i(e,t,n){return!e||e.length===0?K(n.t(`mealPlans.noMealsAddedYet`)):`
    <div class="favorites-item-list">
  ...
  `:``}function bi(e,t,n,r,i){let a=e?.querySelector(`[data-mealplan-search]`),o=e?.querySelector(`[data-mealplan-list]`);!a||!o||a.addEventListener(`input`,()=>{let e=a.value.trim().toLowerCase(),s=t.filter(t=>!e||t.name.toLowerCase().includes(e));o.innerHTML=ui(s,n,r,i)})}function xi(e,t,n,r){let i=e?.querySelector(`[data-mealplan-favorite-search]`),a=e?.querySelector(`[data-mealplan-favorite-results]`);!i||!a||i.addEventListener(`input`,()=>{let e=i.value.trim().toLowerCase(),o=t.filter(t=>!e||t.name.toLowerCase().includes(e));a.innerHTML=gi(o,n,r)})}function Si(e){let t=e.editingRecord??{},n=e.app.i18n,r=e.app.settings?.medication??{},i=r.doseUnit||`mg`,a=r.name||n.t(`route.medication`);return`
  ...
  `}function Ci(e,t,n){if(e.length===0)return K(n.t(`medication.empty`));let r=t.doseUnit||`mg`,i=t.name?`${y(t.name)} &middot; `:``;return`
    <div class="record-list">
      ${e.map(e=>`
            <article class="record-card">
              <div>
                <span class="record-card__meta">${y(n.formatDate(e.administeredAt))} &middot; ${y(wi(e.site,n))}</span>
  ...
  `}function wi(e,t){return t.t({abdomen:`medication.abdomen`,thigh:`medication.thigh`,"upper-arm":`medication.upperArm`,"not-specified":`medication.notSpecified`}[e]??`medication.notSpecified`)}var Ti=[{value:``,labelKey:`settings.sexNotSet`},{value:`female`,labelKey:`settings.sexFemale`},{value:`male`,labelKey:`settings.sexMale`},{value:`other`,labelKey:`settings.sexOther`}],Ei=[{value:`lose-weight`,labelKey:`settings.goalLoseWeight`},{value:`maintain-weight`,labelKey:`settings.goalMaintainWeight`},{value:`gain-muscle`,labelKey:`settings.goalGainMuscle`}],Di=[{value:`sedentary`,labelKey:`settings.activitySedentary`},{value:`light`,labelKey:`settings.activityLight`},{value:`moderate`,labelKey:`settings.activityModerate`},{value:`intense`,labelKey:`settings.activityIntense`}],Oi=[`monday`,`tuesday`,`wednesday`,`thursday`,`friday`,`saturday`,`sunday`].map(e=>({value:e,labelKey:`day.${e}`})),ki=Nn.map(e=>({value:e,labelKey:`settings.administration.${e}`})),Ai=Pn.map(e=>({value:e,labelKey:`settings.frequency.${e}`})),ji=Fn.map(e=>({value:e,labelKey:`settings.doseUnit.${e}`})),Mi=[{key:`calories`,labelKey:`dashboard.calories`},{key:`protein`,labelKey:`dashboard.protein`},{key:`water`,labelKey:`dashboard.water`},{key:`sleep`,labelKey:`dashboard.sleep`},{key:`steps`,labelKey:`dashboard.steps`},{key:`weight`,labelKey:`dashboard.weight`},{key:`medication`,labelKey:`dashboard.medication`},{key:`recentMeals`,labelKey:`dashboard.recentMealsCard`},{key:`recentActivity`,labelKey:`dashboard.recentActivityCard`}];function Ni(e){let t=e.settings??{},n=e.backupStatus??{},r=e.app.i18n,i=e.app.units;return`
    <section class="module-screen" aria-labelledby="settings-title">
      ...
      <form class="settings-form" data-module="settings" data-action="save">
            ...
            <div class="module-form">
              <label>
                ...
                <select name="healthGoal">
                  ${Q(Ei,t.healthGoal??`lose-weight`,r)}
                </select>
              </label>
              <label>
                <span>${r.t(`settings.activityLevel`)}</span>
                <select name="activityLevel">
                  ${Q(Di,t.activityLevel??`moderate`,r)}
                </select>
              </label>
            </div>
          </div>
        ...
        <section class="content-panel">
          ...
          <div class="checkbox-grid">
            ${Mi.map(e=>zi(e,t.dashboardCards,r)).join(``)}
          </div>
        </section>
        ...
        <section class="content-panel">
          ...
          <div class="checkbox-grid">
            ${k.map(e=>Bi(e,t.mealSlots,r)).join(``)}
          </div>
          <div class="form-actions">
      ...
      ${e.developer?.isEnabled?Pi(e.developer,r):``}
    </section>
  `}function Pi(e,t){let n=e.versions??{},r=typeof navigator<`u`&&!!navigator.serviceWorker?.controller;return`
    <section class="content-panel">
    ...
    ${e.confirmingAction?Fi(e.confirmingAction,t):``}
  `}function Fi(e,t){let n=e===`resetFirstLaunch`?`developer.resetFirstLaunchConfirmTitle`:`developer.clearDatabaseConfirmTitle`,r=e===`resetFirstLaunch`?`developer.resetFirstLaunchConfirmMessage`:`developer.clearDatabaseConfirmMessage`;return`
  ...
  `}function Ii(e,t){let n=e.enabled??!0;return`
    <div class="form-subsection">
    ...
    <div class="module-form">
      ...
      <label>
        ...
        <select name="administrationType">
          ${Q(ki,e.administrationType??`injection`,t)}
        </select>
      </label>
      <label>
        <span>${t.t(`settings.frequency`)}</span>
        <select name="frequency">
          ${Q(Ai,e.frequency??`weekly`,t)}
        </select>
      </label>
  ...
  `}function zi(e,t={},n){let r=t?.[e.key]??!0,i=`dashboardCard_${e.key}`;return`
    <label class="checkbox-control">
      <input name="${i}" type="hidden" value="off" />
      <input name="${i}" type="checkbox" ${r?`checked`:``} />
      <span>${n.t(e.labelKey)}</span>
    </label>
  `}function Bi(e,t={},n){let r=t?.[e]??!1,i=`mealSlot_${e}`;return`
    <label class="checkbox-control">
      <input name="${i}" type="hidden" value="off" />
      <input name="${i}" type="checkbox" ${r?`checked`:``} />
      <span>${n.t(`mealSlot.${e}`)}</span>
    </label>
  `}function Vi(e){let t=e.editingRecord??{},n=e.app.i18n,r=e.app.units.weight;return`
  ...
  `}function Hi(e,t){return e.length===0?K(t.t(`weight.empty`)):`
    <div class="record-list">
      ${e.map(e=>`
            <article class="record-card">
              <div>
  ...
  `}var Ui=[{id:`dashboard`,label:`Dashboard`,labelKey:`route.dashboard`,path:`/`,section:`Core`,sectionKey:`section.core`,moduleKey:`dashboard`,render:lr},{id:`meal-journal`,label:`Meal Journal`,labelKey:`route.mealJournal`,path:`/meal-journal`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`meal-journal`,render:ri},{id:`weight-tracking`,label:`Weight Tracking`,labelKey:`route.weightTracking`,path:`/weight-tracking`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`weight-tracking`,render:Vi},{id:`body-measurements`,label:`Body Measurements`,labelKey:`route.bodyMeasurements`,path:`/body-measurements`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`body-measurements`,render:ar},{id:`medication`,label:`Medication`,labelKey:`route.medication`,path:`/medication`,section:`Medication`,sectionKey:`section.medication`,moduleKey:`medication`,render:Si},{id:`favorites`,label:`Favorites`,labelKey:`route.favorites`,path:`/favorites`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`favorites`,hidden:!0,render:Sr,afterRender:Cr},{id:`meal-plans`,label:`Meal Plans`,labelKey:`route.mealPlans`,path:`/meal-plans`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`meal-plans`,hidden:!0,render:oi,afterRender:si},{id:`food-library`,label:`Food Library`,labelKey:`route.foodLibrary`,path:`/food-library`,section:`Tracking`,sectionKey:`section.tracking`,moduleKey:`food-library`,hidden:!0,render:Hr,afterRender:Ur},{id:`settings`,label:`Settings`,labelKey:`route.settings`,path:`/settings`,section:`Support`,sectionKey:`section.support`,moduleKey:`settings`,render:Ni},{id:`backup-restore`,label:`Backup & Restore`,labelKey:`route.backupRestore`,path:`/backup-restore`,section:`Support`,sectionKey:`section.support`,moduleKey:`backup-restore`,render:ir}];function Wi({root:t}){let n=new r({namespace:`HWP`}),i=new e,a=ue({logger:n}),o=tr({persistence:a,eventBus:i}),s=nr({routes:Ui,logger:n}),c=new Ae({eventBus:i,logger:n,modules:o,persistence:a,router:s,storage:a.storage});return ge({eventBus:i,getI18nContext:()=>c.getI18nContext()}),{logger:n,async start(){if(!t)throw Error(`Application root element was not found.`);await c.initialize(),Ve({root:t,viewModel:c}),s.start(),n.info(`Application shell started.`)}}}function Gi(e){`serviceWorker`in navigator&&window.addEventListener(`load`,()=>{navigator.serviceWorker.register(`/sw.js`).catch(t=>{e?.warn(`Service worker registration failed.`,{error:t.message})})})}var Ki=Wi({root:document.querySelector(`#app`)});Ki.start(),Gi(Ki.logger);
//# sourceMappingURL=index-DnP8im9G.js.map